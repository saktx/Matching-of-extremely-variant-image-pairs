...
switch (feature_detector)
    case 'SIFT'
        detector = cv.SIFT('NOctaveLayers',3,'ConstrastThreshold',0.04,'EdgeThreshold',10,'Sigma',1.6);
        descriptor = detector;
        
    case 'SURF'
        detector = cv.SURF('NOctaveLayers',3,'HessianThreshold', 100);
        descriptor = detector;
        
    case 'KAZE'
        detector = cv.KAZE('NOctaveLayers',4,'Threshold',0.001);
        descriptor = detector;
        
    case 'AKAZE'
        detector = cv.AKAZE('NOctaveLayers',4,'DescriptorType','MLDB','Threshold',0.001);
        descriptor = detector;
       
    case 'ORB'
        detector = cv.ORB('MaxFeatures',500000,'FastThreshold',20);
        descriptor = detector;
        
    case 'BRISK'
        detector = cv.BRISK('Octaves',3,'Threshold',30);
        descriptor = detector;
        
    case 'AGAST'
        k1 = cv.AGAST(rgb2gray(I1),'Type','AGAST_7_12d','NonmaxSuppression',true,'Threshold',10);
        k2 = cv.AGAST(rgb2gray(I2),'Type','AGAST_7_12d','NonmaxSuppression',true,'Threshold',10);
        descriptor = cv.SIFT(); % SIFT Descriptor for all Detectors which don't have their own Feature Description Algorithm
        
    case 'FAST'
        k1 = cv.FAST(rgb2gray(I1),'NonmaxSuppression',true,'Threshold',10);
        k2 = cv.FAST(rgb2gray(I2),'NonmaxSuppression',true,'Threshold',10);
        descriptor = cv.SIFT(); % SIFT Descriptor for all Detectors which don't have their own Feature Description Algorithm
        
    case 'MSER'
        detector = cv.MSER('MaxEvolution',200,'EdgeBlurSize',5);
        descriptor = cv.SIFT(); % SIFT Descriptor for all Detectors which don't have their own Feature Description Algorithm
        
    case 'MSD'
        detector = cv.MSDDetector('ThSaliency',250,'NMSRadius',5,'ComputeOrientation', true, 'NScales', 3);
        descriptor = cv.SIFT(); % SIFT Descriptor for all Detectors which don't have their own Feature Description Algorithm
        
    case 'GFTT'
        detector = cv.GFTTDetector('MinDistance',1,'MaxFeatures', 500000,'QualityLevel', 0.01); % Shi-Tomasi Corners
        descriptor = cv.SIFT(); % SIFT Descriptor for all Detectors which don't have their own Feature Description Algorithm
        
    case 'GFTT-H'
        detector = cv.GFTTDetector('HarrisDetector',true,'K',0.04,'MaxFeatures', 500000,'QualityLevel', 0.01);
        descriptor = cv.SIFT(); % SIFT Descriptor for all Detectors which don't have their own Feature Description Algorithm
        
    case 'HARRIS-L'
        detector = cv.HarrisLaplaceFeatureDetector('CornThresh',0.01,'DOGThresh',0.01,'MaxCorners',500000); % Harris Laplace Detector
        descriptor = cv.SIFT();
        
    case 'CenSurE'
        detector = cv.StarDetector('ResponseThreshold',30,'LineThresholdProjected',10,'LineThresholdBinarized',8,'SuppressNonmaxSize',5); % CenSurE
        descriptor = cv.SIFT(); % SIFT Descriptor for all Detectors which don't have their own Feature Description Algorithm
    ...
    for i=1:length(MSERregions1)
        for j=1:length(MSERregions1{i})
            if (mask(MSERregions1{i}(j,2)+1,MSERregions1{i}(j,1)+1) < 1)
                ...
                keypoint(counter).pt = double([(MSERregions1{i}(j,1)+1) (MSERregions1{i}(j,2)+1)]);
    ...
    for i=1:length(MSERregions2)
        for j=1:length(MSERregions2{i})
            if (mask(MSERregions2{i}(j,2)+1,MSERregions2{i}(j,1)+1) < 1)
                ...
                keypoint(counter).pt = double([(MSERregions2{i}(j,1)+1) (MSERregions2{i}(j,2)+1)]);
...
%%% CHECK REPEATABILITY OF THE DETECTOR (*** Repeatability Check Takes Long Time)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Repeatability = 100 * Repeatability
